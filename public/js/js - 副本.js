...
Row.prototype.createRowHtml = function() {
	...
	var newTr = '\
		...
		<td><input class="form-control {a2Class}" id="{a2Id}-{id}" type="text" placeholder="400" value="500"></td>\
	  	...
	  	<td><button type="button" class="btn btn-success ok" id="ok-{id}" style="display: none"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span></button></td>\
...
function queryModel() {
	...
	query.equalTo('size', currentModel.modelSize)
	query.find().then(
		function(results) {
			if (results.length == 1) {
				console.log('查找成功1')
				currentModel.modelClass = results[0]
			} else if (results.length > 1) {
				console.log('结果不唯一')
			} else {
				console.log('无匹配结果')
	...
	.then(function() {
		...
		query.find().then(
			function(results) {
				console.log(currentModel.modelClass)
				if (results.length == 1) {
					console.log('查找成功2')
					currentModel.modelClass = results[0]
				} else if (results.length > 1) {
